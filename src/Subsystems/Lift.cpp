...
float Lift::CalcSafetyTimeOut(int destination) {
	int currentTicks = CurrentLiftPosition();
	int distanceNeeded = 0;
	float ratioinseconds = 0.0;

	// maximum lift time of 5 seconds for calculation of how long to travel from point to setpoint
	distanceNeeded = abs(destination - currentTicks);

	ratioinseconds = ((float) distanceNeeded / MAXLIFTTICKS) * 5.0;

	SmartDashboard::PutNumber("Lift: Calculated timeout: ", ratioinseconds);

	return ratioinseconds;
}
