...
WinchSystem::WinchSystem() :
		Subsystem("WinchSystem") {
	// BEGIN AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DECLARATIONS
	winchMotor = RobotMap::winchSystemwinchMotor;

	// END AUTOGENERATED CODE, SOURCE=ROBOTBUILDER ID=DECLARATIONS

	 Winch_Position = 0; // default and/or starting position for the winch arm
	 	 	 	 	 	 // (this is put here to keep the position of the winch for each command)
...
// sets the winchMotor to a speed from the smartdashboard
	winchMotor->Set(
			SmartDashboard::GetNumber(WINCH_IN_SPEED_STRING,
					WINCH_IN_SPEED_DEFAULT)); // when winch needs to get to a lower point
}

void WinchSystem::WinchOut() {
	// sets the winchMotor to a speed from the smartdashboard
	winchMotor->Set(
			SmartDashboard::GetNumber(WINCH_OUT_SPEED_STRING,
					WINCH_OUT_SPEED_DEFAULT)); // when winch needs to get to a higher point
}
