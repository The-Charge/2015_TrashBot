...
Drive2Feet::Drive2Feet() {
	...
	SmartDashboard::PutNumber (CLOSED_LOOP_RAMP_RATE_DASHBOARD_KEY, CLOSED_LOOP_RAMP_RATE_DEFAULT);
...
void Drive2Feet::Initialize() {
	...
	float i = SmartDashboard::GetNumber(INTEGRAL_CONSTANT_DAHSBOARD_KEY , INTEGRAL_CONSTANT_DEFAULT);
	float rampRate = SmartDashboard::GetNumber(CLOSED_LOOP_RAMP_RATE_DASHBOARD_KEY , CLOSED_LOOP_RAMP_RATE_DEFAULT);
	float maxvoltage = SmartDashboard::GetNumber(MAX_PERCENT_DASHBOARD_KEY, MAX_PERCENT_DEFAULT);
...
void Drive2Feet::Execute() {
	SmartDashboard:: PutNumber(CLOSED_LOOP_ERROR_DASHBOARD_KEY,frontLeftPIDController->GetError());
	SmartDashboard :: PutNumber (ENCODER_POSITION_DASHBOARD_KEY, Robot:: driveTrain -> CANTalonLeftFront -> GetEncPosition());
}
