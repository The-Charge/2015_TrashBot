...
LiftXTicks::LiftXTicks(int t, float s) {
	...
	SmartDashboard::PutNumber(MAX_VALUE_TICKS, Robot::lift->MAX_LIFT_TICKS);
...
void LiftXTicks::Initialize() {
	...
	this->SetTimeout(timeout); // sets timeout for safety
...
void LiftXTicks::Execute() {
	LIFT_ENCODER_TICKS = Robot::lift->CurrentLiftPosition();
	SmartDashboard::PutNumber(LIFT_ENCODER_VALUE, LIFT_ENCODER_TICKS);
...
void LiftXTicks::End() {
	Robot::lift->speedController->Set(0); //originally a lift command
